...
request_url = "http://127.0.0.1:5000/api/v1/user/account"
...
def process_register():
	...
	user_details = {
		...
		, "body"	:	{
			...
			, "dob"			:	datetime.strftime(datetime.strptime(request.form["dob"], '%Y-%m-%d'), '%d-%m-%Y')
...
def update_account_name():
	request_data	=	{
		...
		, "body": {
			...
			, "new_value": request.form["updated_name"]
		}
	}
	...
	if response_update_name.status_code == 204:
		...
		getAccountDetailsToSession()
		# updated new values in the session
...
def update_account_passsword():
	request_data	=	{
		...
		, "body": {
			...
			, "new_value": request.form["updated_password"]
		}
	}
