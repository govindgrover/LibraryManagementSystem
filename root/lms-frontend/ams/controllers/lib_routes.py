...
# API Endpoint as defined in the LMS - API
request_url_for_adding_master		=	"http://127.0.0.1:5000/api/v1/book/master"
request_url_for_filtering_masters	=	"http://127.0.0.1:5000/api/v1/fetch/details/master"
request_url_for_updating_masters	=	"http://127.0.0.1:5000/api/v1/book/master"

request_url_for_adding_book			=	"http://127.0.0.1:5000/api/v1/book/operation"
request_url_for_filtering_books		=	"http://127.0.0.1:5000/api/v1/fetch/details/books"
request_url_for_updating_book		=	"http://127.0.0.1:5000/api/v1/book/operation/"

request_url_for_book_issuing		=	"http://127.0.0.1:5000/api/v1/interact/librarian"
...
def process_modify_master_with_field(master, field, is_allowed: bool = False):
	...
	if field not in ("name", "bio", "active", "delete"):
		return jsonify({
			"status": "bad"
			, "msg": [
	...
	params_to_update["body"][master + "_id"]		=	request.form[master + "_id"]
	# provided required ID to process further

	if field == "name":
		params_to_update["body"]["field_to_update"]	=	"name"
		params_to_update["body"]["new_value"]		=	request.form["updated_name"]
	elif field == "bio":
		params_to_update["body"]["field_to_update"]	=	"bio"
		params_to_update["body"]["new_value"]		=	request.form["updated_bio"]
	elif field == "active":
		params_to_update["body"]["field_to_update"]	=	"active"
		params_to_update["body"]["new_value"]		=	1 if ("updated_active_status" in request.form and request.form["updated_active_status"] == "on") else 0
...
@login_required()
@restrict_access({0, 1})
def process_get_books(is_allowed :bool = False):
	if not is_allowed:
		session["is_access_denied"] = True
		return redirect("/access-denied")
...
def process_modify_book_with_field(field, is_allowed: bool = False):
	...
	if field == "title":
		...
		params_to_update["body"]["new_value"]		=	request.form["updated_title"]
	elif field == "desc":
		params_to_update["body"]["field_to_update"]	=	"book_desc"
		params_to_update["body"]["new_value"]		=	request.form["updated_desc"]
	elif field == "price":
		params_to_update["body"]["field_to_update"]	=	"book_price"
		params_to_update["body"]["new_value"]		=	request.form["updated_price"]
	elif field == "active":
		params_to_update["body"]["field_to_update"]	=	"active"
		params_to_update["body"]["new_value"]		=	1 if ("updated_active_status" in request.form and request.form["updated_active_status"] == "on") else 0
...
def process_get_issue_requests(is_allowed :bool = False):
	...
	data_fetched = requests.get(
		"http://127.0.0.1:5000/api/v1/interact/librarian"
		, json	=	{
...
def process_grant_issue_requests(is_allowed :bool = False):
	...
	data_fetched = requests.post(
		...
		, json	=	{
			...
			, "body"	:	{
				"user_id"	:	request.form["book_requested_by"]
	...
	data_fetched = requests.post(
		...
		, json	=	{
			...
			, "body"	:	{
				"user_id"			:	request.form["book_requested_by"]
				, "book_id"			:	request.form["requested_book_id"]
				, "current_cost"	:	request.form["written_cost"]
	...
	if data_fetched.status_code == 206:
		...
		return jsonify({
			...
			, "msg"	:	[
				...
				, "Kindly copy the Access Token from the alert box!!"
			]
			, "data" : data_fetched.json()["data"]
		})
...
def process_revoke_issued_book(is_allowed :bool = False):
	...
	data_fetched = requests.post(
		...
		, json	=	{
			...
			, "body"	:	{
				"user_id"	:	request.form["issued_to_id"]
				, "book_id"	:	request.form["issued_book_id"]
			}
		}
		, headers	=	{
			"Authorization" : "Bearer {}".format(
